...
class DupireLocalVolSurface:
    ...
    def get_local_vol(self, maturity: float, strike: float) -> float:
        local_variance = self.get_local_variance(maturity, strike)
        return float(np.sqrt(max(local_variance, 0.0)))
    ...
    def get_local_variance(self, maturity: float, strike: float) -> float:
        ...
        c = self._call_price(maturity, strike)

        if maturity - dt <= 0:
            dcdt = (self._call_price(maturity + dt, strike) - c) / dt
        else:
            dcdt = (
        ...
        r = self._zero_rate(maturity)
        q = self.dividend_yield
        numerator = dcdt + (r - q) * strike * dc_dk + q * c
    ...
    def _call_price(self, maturity: float, strike: float) -> float:
        ...
        r = self._zero_rate(maturity)
        q = self.dividend_yield
        forward = self.spot * np.exp((r - q) * maturity)
